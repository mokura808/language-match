...
function setDate() {
  ...
  deadline =  document.getElementById("deadline").value;
    ...
    $.post('/dateSumbitted',{open:open, deadline:deadline, open:open});
}
/*
var confirmButton = document.getElementById("confirmdate");
...
viewButton.addEventListener("click", function(evento) {
    ...
    $.post('/viewData', function(data, status) {
        if (status == "success") {
            ...
            document.getElementById('viewdata').appendChild(link);
            link.click();
...
viewAllUsersButton.addEventListener("click", function(evento) {
    ...
    $.post('/viewAllUsers', function(data, status) {
        ...
        if (status == "success") {
            ...
            var csv = '';
            // add column titles at top of file
            csv += ',Name,Email,Password,Bio,Availability,Class Year\n';
            //csv += ',Key,Room Name\n';
            var counter = 1;
            data.forEach(function(element) {
              // for each entry, write it to a line of the csv file
...
$.get('/getLanguages', function(data, status) {
    ...
    if (status === 'success') {
        ...
        data.forEach(function(element, index) {
            if (index === length - 1) { // last language; no comma
