...
conn.query('CREATE TABLE if not exists deadline(date TEXT)', function(error, data) {
    if (error) {
        console.log(error)
    } else {
...
function handledate(request, response) {
    ...
    conn.query('SELECT date FROM deadline', function(error, data) {
          if (error) {
            ...
            if(datecount ==0){
                  conn.query('INSERT INTO deadline VALUES(?)', [request.body.deadline], function(error, data) {
                      if (error) {
                        console.log(error);
                        } else {
                        console.log("server date updated to");
                        console.log(request.body.deadline);
                    }
                });
            }else if(datecount==1){
                 conn.query('UPDATE deadline SET date = ?', [request.body.deadline], function(error, data) {
                      if (error) {
                        console.log(error);
                        } 
                });
...
function resetData(request, response) {
    ...
    conn.query('DROP TABLE IF EXISTS teacher', function(error, data) {
       ...
       conn.query('DROP TABLE IF EXISTS deadline', function(error, data) {
        if (error) {
            console.log(error)
        } else {
        }
...
//hardcore check the variables as I am making the no-match-emails
    ...
    let matched_list = ['melokura3@gmail.com']
    let noMatch_list = ['melokura3@gmail.com']
    ...
    transporter.sendMail(mailOptions_match, function(error, info){
        if(error) {
            console.log(error);
        } else {
    ...
    var mailOptions_noMatch = {
        from: 'melokura3@gmail.com',
        to: 'melia_okura@brown.edu', //replace later with list of users with no match
        subject: 'Language Match Update',
        //text: 'unfortunately, you do not have a match :('
        html: { path: __dirname + "/public/noMatch_email.html"}
    }

    transporter.sendMail(mailOptions_noMatch, function(error, info){
        if(error) {
            console.log(error);
        } else {
