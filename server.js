...
conn.query('CREATE TABLE if not exists deadline(date TEXT, open INTEGER)', function(error, data) {
    if (error) {
        console.log(error)
    } else {
...
function handledate(request, response) {
    ...
    conn.query('SELECT date FROM deadline', function(error, data) {
          if (error) {
            ...
            if(datecount ==0){
                  conn.query('INSERT INTO deadline(date,open) VALUES(?,?)', [request.body.deadline, request.body.open], function(error, data) {
                      if (error) {
                        console.log(error);
                        } else {
                 ...
                 conn.query('UPDATE deadline SET date = ?', [request.body.deadline], function(error, data) {
                   ...
                   conn.query('UPDATE deadline SET open = ?', [request.body.open], function(error, data) {
                      if (error) {
                        console.log(error);
                        } 
                });
...
app.get('/disclaimer', function(request, response) {
    response.sendFile(__dirname + "/public/disclaimer.html");
});
